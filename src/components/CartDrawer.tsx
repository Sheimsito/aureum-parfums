 ...
 * Handles item management and displays a simulated order confirmation.
 */
export const CartDrawer: React.FC = () => {
  const { items, isOpen, setOpen, updateQuantity, removeItem, clearCart, count } = useCart();
  const [orderConfirmed, setOrderConfirmed] = useState(false);

  const subtotal = items.reduce((sum, item) => sum + item.product.price * item.quantity, 0);

  /**
   * Simulates order placement: shows confirmation screen and clears the cart.
   */
  ...
  /**
   * Resets the confirmation state and closes the drawer.
   */
  const handleClose = () => {
    setOpen(false);
      ...
      >
        ...
        <div className="flex justify-between items-center mb-8 border-b border-outline-variant pb-4">
          <div>
            <h2 className="font-headline-sm text-headline-sm text-primary dark:text-on-primary">
              {orderConfirmed ? 'Pedido Confirmado' : 'Tu Bolsa'}
            </h2>
            <p className="font-body-md text-body-md text-on-surface-variant dark:text-on-primary-container">
              {orderConfirmed ? 'Tu pedido ha sido procesado' : 'Tu selección de fragancias'}
            </p>
          </div>
          <button
        ...
        {orderConfirmed ? (
          <div className="flex flex-col items-center justify-center flex-grow text-center px-4">
            <span className="material-symbols-outlined text-6xl mb-6 text-primary dark:text-on-primary">
              check_circle
            </span>
            <h3 className="font-headline-sm text-headline-sm text-primary dark:text-on-primary mb-3">
              ¡Gracias por tu compra!
            </h3>
            <p className="font-body-md text-body-md text-on-surface-variant dark:text-on-primary-container mb-2">
              Hemos recibido tu pedido y lo estamos preparando con cuidado. Recibirás una confirmación en breve.
            </p>
            <p className="font-label-sm text-label-sm text-on-surface-variant/60 mb-10">
              Pedido #{Math.random().toString(36).substring(2, 10).toUpperCase()} · Entrega estimada: 3–5 días hábiles
            </p>
            <button
              onClick={handleClose}
              className="w-full border border-outline-variant text-primary dark:text-on-primary font-label-lg text-label-lg uppercase tracking-widest py-4 hover:bg-surface-container-high transition-colors duration-300 cursor-pointer"
            >
            ...
            <div className="flex-grow overflow-y-auto pr-2 space-y-6">
              {items.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-64 text-center">
                  ...
                  <p className="font-body-md text-on-surface-variant">Tu bolsa está vacía.</p>
                  <button
                    onClick={handleClose}
            ...
            {items.length > 0 && (
              <div className="mt-8 pt-6 border-t border-outline-variant space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-label-sm text-on-surface-variant">{count}</span>
                </div>
